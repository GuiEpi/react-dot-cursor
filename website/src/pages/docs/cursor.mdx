...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>string</td>
      <td>auto</td>
      <td>Default color (supports multiple formats)</td>
    </tr>
  </tbody>
</table>
...
Change the default cursor color. The value supports **multiple color formats** including HSL, RGB, OKLCH, hex, and named colors. By default, the cursor uses the HSL values from the <a href="https://ui.shadcn.com/colors" target="_blank" rel="noopener noreferrer">Tailwind CSS color palette</a>:
...
```jsx
// HSL (with or without function wrapper)
<Cursor defaultColor="349, 80%, 59%" />
<Cursor defaultColor="hsl(349, 80%, 59%)" />

// RGB (with or without function wrapper)
<Cursor defaultColor="255 64 129" />
<Cursor defaultColor="rgb(255, 64, 129)" />

// OKLCH (with or without function wrapper)
<Cursor defaultColor="0.7 0.15 330" />
